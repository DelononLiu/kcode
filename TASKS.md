...
| P13-03 | 会话版本管理 — 时间线 + Git 关联 | ⏸️ 已延期 → Backlog | P2 |
...
**涉及文件**:
- `src/store/__tests__/TaskStore.test.ts` — **新建**：5 tests，覆盖 CRUD
- `src/kcodeView/__tests__/MessageRouter.test.ts` — 新增 3 边界用例
- `src/kcodeView/webview/sidebar.ts` — 3 渲染状态测试（需 jsdom）
- `src/core/__tests__/AgentService.test.ts` — 新增 4 异常路径用例
- `vitest.config.ts` — 新增 jsdom 环境配置

**调研步骤**:
1. 运行 `npm test` 确认当前 47 个测试全部通过
...
**调研结果**:
- 当前测试基线：**47 tests, 4 files**（TaskFlow 21 + externalPrompts 6 + MessageRouter 6 + AgentService 14）
- vitest 配置在 `vitest.config.ts`，无 jsdom 环境（需添加 `environment: 'jsdom'`）
- `TaskStore` 的 `vscode.Memento` 接口简单（`get<T>()`/`update()`），内存 mock 即可测试
- `sidebar.ts` 使用 `acquireVsCodeApi` + DOM 操作，需 vitest 配置 jsdom
- `AgentService` 已有完整 mock（AcpClient/OpenAIAgent/vscode），可借鉴
- `KCodePanel` 依赖 WebViewPanel/Webview 等复杂接口，mock 成本高，暂不纳入

**状态**: ✅ 已完成

**验收标准**：测试总数 62（从 47 增至 62，新增 15），覆盖场景：TaskStore CRUD（5）、MessageRouter 边界（9，含 3 个新增）、sidebar 渲染状态（3）、AgentService 异常路径（18，含 4 个新增）。`tsc --noEmit` 零错误，`npm test` 全部通过。
...
**背景**: KCode 当前 10 个配置项全部定义在 `package.json` 的 `contributes.configuration` 中（agentName/agentArgs/openaiApiKey/openaiModel/agentPath/openaiBaseUrl/acpLogEnabled/acpLogMaxGlobal/acpLogMaxTask/githubToken）。Kilo Code 使用双层架构：简单偏好走 VS Code settings，复杂配置走文件 `kilo.jsonc` + 自定义 WebView 面板（14 tab + draft/commit 保存模式）。

| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P16-01 | 配置 Schema 设计 — kcode.jsonc 格式定义 | ⬜ 未开始 | P1 |
| P16-02 | ConfigService — JSONC 文件读/写/监听 | ⬜ 未开始 | P1 |
| P16-03 | Settings Panel WebView — 扩展侧 Provider + 路由 | ⬜ 未开始 | P1 |
| P16-04 | Settings Panel WebView — 前端 UI（多 tab + draft/commit 保存） | ⬜ 未开始 | P1 |
| P16-05 | 配置迁移 — 现有 VS Code 配置项迁移到文件 + 向后兼容 | ⬜ 未开始 | P1 |
| P16-06 | 配置导入/导出 — 完整配置 JSON 导入导出 | ⬜ 未开始 | P2 |
...
**调研步骤**:
1. 调研 Kilo Code `Config` interface + `kilo.jsonc` 格式（位于 `webview-ui/src/types/messages/config.ts`）
2. 整理 KCode 当前 10 个配置项（列自 `package.json`）
3. 设计 KCode 的独立配置 schema，覆盖 Agent / Provider / UI / Log / GitHub 等域
...
**调研步骤**:
1. 调研 Kilo Code `config-file.ts` 的全局/项目两级查找逻辑
2. 调研 `jsonc-parser` 库用法（保留注释，非破坏性写入）
3. 设计 `ConfigService` 接口：`load() / get(key) / set(key, value) / watch(callback) / save()`
...
**调研步骤**:
1. 调研 Kilo Code `SettingsEditorProvider.ts` — WebviewPanel 创建 + 消息路由
2. 调研 Kilo Code `KiloProvider.ts` — config 消息在 extension ↔ webview 间的传递
3. 设计 `SettingsProvider`：管理 Panel 生命周期 + `updateConfig`/`configUpdated`/`configUpdateFailed` 消息
...
**调研步骤**:
1. 调研 Kilo Code `Settings.tsx`（14 tab 布局）+ `config.tsx`（draft/commit 模式）
2. 设计 tab 分区：Agent / Provider / ACP Log / Display / GitHub / Experimental / About
3. 决定前端技术：KCode WebView 使用 Vanilla JS，参考 Kilo 的 `draft` + `isDirty` + save bar 模式
...
**调研步骤**:
1. 清点当前通过 `vscode.workspace.getConfiguration('kcode')` 读取配置的全部代码
2. 列出兼容策略：初次启动时从 VS Code 配置导出到 `kcode.jsonc`，后续优先读文件
3. 确认 VS Code `contributes.configuration` 是否可移除或精简
...
**调研步骤**:
1. 调研 Kilo Code `settings-io.ts` 的导入导出实现
2. 设计 KCode 的完整配置 JSON 导出格式 + 导入校验

---

## Backlog

_当前不排期、留待后续做的任务。取自 Phase 13/14 中涉及存储基础设施的重构和知识沉淀系列。_

| 任务 | 说明 | 优先级 |
|------|------|--------|
| P13-03 | 会话版本管理 — 时间线 + Git 关联 | P2 |
| P14-01 | 存储层重构 — Memento → SQLite | P0 |
...
### P14-01: 存储层重构 — Memento → SQLite

**涉及文件**:
- `src/store/TaskStore.ts` — 全部方法重写为 SQLite 驱动
- `src/store/database.ts` — **新建**：SQLite 初始化 + schema 定义
- `src/types/index.ts` — 新增数据库行对应的接口
- `package.json` — 新增 `sql.js` 依赖
- `src/extension.ts` — 初始化时创建 Database 实例，传入 TaskStore

**调研步骤**:
1. 确认 `sql.js` 在 VS Code 扩展 WebView/Node 环境的兼容性
2. 调研 vsce 打包 wasm 文件的配置（`sql.js` 依赖 `sql-wasm.wasm`）
3. 确认现有 TaskStore 的全部方法清单，设计迁移映射表
...
CREATE TABLE tasks (
    ...
    phase TEXT NOT NULL DEFAULT 'demand',
    workspace_id TEXT,
    container_id TEXT,
    group_name TEXT,
    ...
    source_json TEXT,
    confirmed_items_json TEXT,
    pending_items_json TEXT,
    plan_steps_json TEXT,
    hooks_json TEXT,
    node_message_ids_json TEXT
);
...
CREATE INDEX idx_messages_task ON messages(task_id, sort_order);
...
CREATE INDEX idx_tool_groups_task ON tool_groups(task_id, sort_order);
...
CREATE INDEX idx_tool_items_group ON tool_items(group_id, sort_order);
CREATE INDEX idx_tool_items_task ON tool_items(task_id, sort_order);
...
CREATE INDEX idx_knowledge_task ON knowledge_entries(task_id);
```

**迁移策略**:
- Memento 的 `tasks` key → `tasks` 表全量 insert
- `messages_{taskId}` key → `messages` 表批量 insert
- 迁移脚本在第一次检测到 `tasks` 表为空但 Memento 有数据时触发
- 后续读写全部走 SQLite，Memento 只作为 fallback 兼容层（逐步废弃）

**存储位置**: `ExtensionContext.storageUri` 下创建 `kcode.db`

**状态**: ⏸️ 已延期

**验收标准**：现有 `getTasks/addTask/deleteTask/getMessages/addMessage` 等全部 CRUD 方法工作在 SQLite 上，存量数据无缝迁移，`npm test` 全通过。
...
**涉及文件**:
- `src/store/TaskStore.ts` — 新增 `addToolGroup()` / `addToolItem()` / `getTaskToolGroups()` 方法
- `src/kcodeView/KCodePanel.ts` — 收到 tool_call/tool_call_update 时写入 tool_groups + tool_items
- `src/types/index.ts` — 新增 `ToolGroup`、`ToolItem` 接口（如果需要）
...
**只存原始业务数据**:
- `tool_items.detail` 存原始输出内容（bash 的 stdout+stderr、read 的文件内容等）
- 不存 TabCard 拆分信息、不存头部分段、不存选中态
- WebView 加载时实时计算：`getTaskToolGroups(taskId)` → 按 sort_order 连续分组 → 前端算 TabCard
...
[
  {
    "type": "decision",
    "title": "选择 sql.js 而非 better-sqlite3",
    "content": "## 背景\n需要嵌入式数据库...\n\n## 结论\n使用 sql.js (WASM) 避免 native 编译的跨平台问题。",
...
**验收标准**：review 阶段 AI 自动输出知识摘要，存入 knowledge_entries 表；新任务构建 prompt 时自动注入相关历史知识；右栏知识区显示当前任务关联条目。
...
<div class="op-knowledge-entry" data-entry-id="xxx">
    ...
    <span class="op-knowledge-title">选择 sql.js 方案</span>
    <div class="op-knowledge-tags">
        <span class="op-tag">#database</span>
        <span class="op-tag">#architecture</span>
    </div>
</div>
```
...
- 当任务完成且 knowledge_entries 为空时，显示「该任务暂无知识沉淀」+ 提示 AI 在 review 阶段自动生成
