...
_当前不排期、留待后续做的任务。取自 Phase 14 中涉及工具数据持久化和知识沉淀系列。_
...
## Phase 17: 我的任务 — 跨工作区任务看板

_目标：构建独立的「我的任务」编辑器面板，作为跨工作区的全局任务看板。与侧边栏「任务列表」（当前工程、仅未归档）形成两个互补视图。存储从 workspaceState 迁移到 `~/.local/share/kcode`。原有 Dashboard（P10-04）和「工作台」按钮合并到「我的任务」中，不再独立存在。_
...
| **我的任务**（独立面板） | 所有工作区（跨工程） | 全部任务（含归档） | 表格: tab 切换「进行中/待验收/已归档/全部」 | 侧边栏底部「📋 我的任务」按钮 |

### 生命周期规则

1. 新建任务 → 出现在侧边栏「任务列表」中（当前工程、未归档）
2. 归档操作 → 任务从侧边栏「任务列表」消失，但在「我的任务」的「已归档」tab 中仍可见
3. 恢复归档 → 任务重新出现在侧边栏「任务列表」中
4. 「我的任务」始终展示所有工作区的全部任务（含已归档），不受侧边栏过滤影响

| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P17-01 | 文件存储层 — TaskStore 迁移到 ~/.local/share/kcode | ⬜ 未开始 | P0 |
| P17-02 | 跨工作区任务索引 — 全局任务注册表 | ⬜ 未开始 | P0 |
| P17-03 | 我的任务面板 — 独立 WebviewPanel + 表格视图 | ⬜ 未开始 | P1 |
| P17-04 | 归档/恢复归档 — 完整生命周期操作 | ⬜ 未开始 | P1 |
| P17-05 | 侧边栏底部精简 — 移除「我的项目」，保留「我的任务」入口 | ⬜ 未开始 | P1 |
| P17-06 | 移除旧 Dashboard 代码 — P10-04 + 控制区「📊 工作台」按钮 | ⬜ 未开始 | P1 |

---

### P17-01: 文件存储层 — TaskStore 迁移到 ~/.local/share/kcode

**涉及文件**: _待调研_

**调研步骤**:
1. 读 `src/store/TaskStore.ts` 确认当前 Memento 接口和方法签名
2. 读 `src/extension.ts` 确认 store 初始化流程
3. 确认 `~/.local/share/kcode/` 目录结构与文件命名规范

**设计要点**:

- **存储位置**: `~/.local/share/kcode/tasks/` 目录，每个工作区一个 JSON 文件
  - 文件命名: `{workspace_hash}.json`（路径 hash 避免特殊字符）
  - 全局索引: `~/.local/share/kcode/index.json`（记录所有已知工作区路径 + hash 映射）
- **文件格式**: JSON（非 JSONC），内部结构：
  ```json
  {
    "workspacePath": "/home/user/projects/kcode",
    "tasks": [...],
    "containers": [...],
    "assistant_messages": [...],
    "messages_{taskId}": [...],
    "meta": { "lastModified": 1715937600000, "version": 1 }
  }
  ```
- **读写策略**: 启动时全量加载，增删改时全量写入（MVP 阶段数据量小，无需增量）
- **向后兼容**: 首次启动检测 `workspaceState` 中是否有旧数据，有则自动迁移到文件，迁移后清空 workspaceState 中的旧 key
- **接口保持**: `ITaskStore` 接口不变，仅实现层从 `Memento` 切换为文件读写

**状态**: ⬜ 未开始

**验收标准**: 任务数据持久化到 `~/.local/share/kcode/`，重启 VS Code 后数据完整恢复。旧 workspaceState 数据自动迁移。

---

### P17-02: 跨工作区任务索引 — 全局任务注册表

**涉及文件**: _待调研_

**调研步骤**:
1. 读 P17-01 确定后的文件存储结构
2. 确认「我的任务」面板需要查询的字段（标题/状态/阶段/工作区/归档/时间）

**设计要点**:

- **全局索引文件**: `~/.local/share/kcode/index.json`，结构：
  ```json
  {
    "workspaces": [
      { "hash": "a1b2c3", "path": "/home/user/projects/kcode", "lastOpened": 1715937600000 },
      { "hash": "d4e5f6", "path": "/home/user/projects/other", "lastOpened": 1715851200000 }
    ]
  }
  ```
- **聚合查询**: `getAllTasks(filter?)` — 遍历所有已注册工作区的数据文件，按筛选条件（状态/阶段/归档/搜索关键词）返回聚合结果
- **工作区注册**: VS Code 激活时自动注册当前工作区到 `index.json`
- **懒加载**: 仅在打开「我的任务」面板时才加载非当前工作区的完整数据
- **缓存**: 当前工作区数据常驻内存（与现有行为一致），其他工作区按需加载后缓存 30 秒

**状态**: ⬜ 未开始

**验收标准**: 「我的任务」可列出所有已知工作区的任务。切换工作区后新 workspace 自动注册。

---

### P17-03: 我的任务面板 — 独立 WebviewPanel + 表格视图

**涉及文件**: _待调研_

**调研步骤**:
1. 读 `src/kcodeView/KCodePanel.ts` 了解 WebviewPanel 创建模式
2. 读 `src/kcodeView/webview/dashboard-panel` 现有 dashboard 渲染代码（将被取代）
3. 读 `src/kcodeView/templates/chatPanelHtml.ts` 了解 HTML 模板模式
4. 读 `src/kcodeView/SettingsProvider.ts` 了解独立 WebviewPanel 实现
...
│  │ 任务               工作区    状态    阶段    时间     │  │
│  │──────────────────────────────────────────────────────│  │
│  │ P17-01 文件存储层   kcode    active  execute  2h ago  │  │
│  │ P16-02 ConfigSvc    kcode    in_rev  review   昨天    │  │
│  │ Bug fix #42         other    active  execute  3d ago  │  │
│  │ OAuth 登录          archived archived —        1w ago  │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                            │
│  共 12 个任务 · 3 个工作区                                  │
└────────────────────────────────────────────────────────────┘
...
- **入口**: 侧边栏底部「📋 我的任务」按钮 → 打开独立 WebviewPanel（类似 Settings 面板）
- **Tab 栏**:
  - 进行中 — 状态为 `active`（含 `in_review`）的未归档任务
  - 待验收 — 状态为 `in_review` 的任务
  - 已归档 — 所有 `archived: true` 的任务
  - 全部 — 所有任务（含归档）
- **表格列**: 任务标题（点击打开对应工作区 → 对应任务面板）、工作区名、状态、阶段、最后活跃时间
- **搜索**: 输入框过滤任务标题
- **操作**: 每行右侧「恢复归档」按钮（仅已归档 tab 显示）
- **无任务时**: 显示引导文字「暂无任务，在侧边栏创建第一个任务」

**与 KCodePanel 的关系**:
- 我的任务面板为独立 WebviewPanel，不占用主 KCodePanel
- 点击表格中的任务行 → 如果该工作区已打开 → 切换到 KCodePanel 并 `loadTask(taskId)`；如果工作区未打开 → `vscode.window.showInformationMessage('请先打开对应工作区')`
- 面板关闭时不做自动保存（数据已在存储层持久化）

**状态**: ⬜ 未开始

**验收标准**: 侧边栏底部「📋 我的任务」打开独立面板，4 tab 切换展示所有工作区的任务表格。搜索可过滤。点击任务行可跳转到对应聊天面板。
...
**调研步骤**:
1. 读 `src/types/index.ts` 确认 Task.archived 字段
2. 读 `src/store/TaskStore.ts` 确认归档相关方法
3. 读 `src/kcodeView/webview/sidebar.ts` 确认右键菜单归档逻辑

**设计要点**:

- **归档操作**: 任务右键菜单「归档」→ `task.archived = true` → 任务从侧边栏「任务列表」移除，但在「我的任务」面板的「已归档」tab 中可见
- **恢复归档**: 「我的任务」面板「已归档」tab 中每行有「恢复」按钮 → `task.archived = false` → 任务重新出现在侧边栏
- **归档语义**:
  - 归档 ≠ 删除，数据完整保留
  - 归档的任务不参与侧边栏渲染、不参与项目进度统计
  - 归档的任务对话历史完整保留，恢复后可继续对话
- 当前 sidebar.ts:627-634 的归档实现走的是 `deleteTask`，需修复为设置 `archived: true`
...
**调研步骤**:
1. 读 `src/kcodeView/KCodeSidebarProvider.ts` 确认 footer 按钮和消息处理
2. 读 `src/kcodeView/webview/sidebar.ts` 确认按钮事件绑定
...
┌────────────────────┐               ┌────────────────────┐
│ 🏗️ 我的项目        │               │ 📋 我的任务        │  ← 打开独立面板
│ 📋 我的任务        │               │ ✨ AI Agent      │  ← 打开 Settings 面板 Agent Tab
│ 📚 我的知识库       │               │ ⚙️ 设置          │  ← 打开 Settings 面板
│ ⚙️ 设置            │               └────────────────────┘
└────────────────────┘
```

- **移除**: `🏗️ 我的项目`（项目改为侧边栏中部的树形聚合 + 「我的任务」面板底部项目进度条）
- **变更**: `📋 我的任务` → 打开独立「我的任务」面板（WebviewPanel），而非当前跳到 KCode 面板
- **变更**: `📚 我的知识库` → 暂改为 `✨ AI Agent`（打开 Settings 面板 Agent tab），知识库入口后续再加
- **保留**: `⚙️ 设置` — 打开 Settings 面板

**状态**: ⬜ 未开始

**验收标准**: 侧边栏底部仅 3 个按钮（我的任务/AI Agent/设置），点击「我的任务」打开独立面板。
...
**调研步骤**:
1. 读 `src/kcodeView/templates/chatPanelHtml.ts` 定位 `#dashboard-panel` / `#btn-dashboard`
2. 读 `src/kcodeView/webview/app.ts` 定位 `renderDashboardPanel` / `showDashboard` 等渲染函数
3. 读 `src/kcodeView/KCodePanel.ts` 定位 dashboard 相关消息处理

**设计要点**:

- **移除**: `#dashboard-panel` HTML 及其所有 CSS
- **移除**: 控制区工具栏「📊 工作台」按钮（`#btn-dashboard`）
- **移除**: `renderDashboardPanel()` / `showDashboard` 等函数
- **移除**: KCodePanel 中 dashboard 消息处理逻辑
- **取代**: 无任务选中时的默认视图改为空态引导「选择左侧任务开始对话，或点击底部📋 我的任务查看全局」
