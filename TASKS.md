...
## Phase 17: 我的任务 — 任务看板面板

_目标：构建独立的「我的任务 + 我的项目」编辑器面板，与侧边栏「任务列表」（当前工程、仅未归档）形成两个互补视图。存储从 workspaceState 迁移到 `~/.local/share/kcode`。原有 Dashboard（P10-04）和「工作台」按钮合并到「我的任务」中，不再独立存在。_
...
| P17-03 | 我的任务面板 — 双主Tab（📋 我的任务 / 📦 我的项目），UI 已实现需接真实数据 | ⬜ 未开始 | P1 |
...
- **存储位置**: `~/.local/share/kcode/tasks/` 目录，每个工作区一个 JSON 文件
  - 文件命名: `{workspace_hash}.json`（路径 hash 避免特殊字符，仅当前工作区）
...
**涉及文件**:
- `src/kcodeView/MyTasksProvider.ts` — 扩展侧：`_sendTaskData()` 发送真实任务数据替代 mock
- `src/kcodeView/webview/myTasksApp.ts` — WebView 侧：搜索过滤、tab 切换、点击跳转
- `src/store/TaskStore.ts` — `getTasks()` / `getContainers()` 数据源

**现状**（UI 已实现）:

`MyTasksProvider` + `myTasksApp.ts` 已完整实现面板 UI，结构如下：
...
│  │ 任务               状态    阶段    时间           │  │
│  │──────────────────────────────────────────────────│  │
│  │ P17-01 文件存储层   active  execute  2h ago       │  │
│  │ P16-02 ConfigSvc    in_rev  review   昨天         │  │
│  │ ...                                               │  │
│  └──────────────────────────────────────────────────┘  │
│  共 N 个任务                                            │
└────────────────────────────────────────────────────────┘
...
- **主Tab**: 📋 我的任务 / 📦 我的项目（通过 `switchPrimaryTab()` 切换）
- **任务状态Tab**: 进行中 / 待验收 / 已归档 / 全部
- **项目Tab**: 展示容器列表（📦 名称 + ✕ 删除），隐藏搜索栏
- **搜索**: 输入框可过滤任务标题

**当前局限**:
- 任务数据使用硬编码 `MOCK_TASKS`，未连接真实 `TaskStore`
- 点击任务行弹出 `（逻辑待 Phase 17 后端实现后接入）` 提示
- 项目数据已通过 `_sendProjectData()` 发送真实容器数据到 WebView

**待实现**:
1. 替换 mock 数据为 `TaskStore.getTasks()`（当前工作区）
2. `_sendTaskData()` 发送真实任务列表
3. 点击任务行 → 关闭当前面板 → 加载对应任务的 KCodePanel
4. 搜索从 mock 过滤改为真实数据过滤
5. 任务 Tab 切换（进行中/待验收等）基于真实 task.status 过滤
6. 面板关闭时不做自动保存（数据已在 store 持久化）
