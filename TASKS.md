...
| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P21-01 | device-executor 执行内核 — 独立 CLI 工具，完全解耦业务逻辑 | 🛠️ 部分实现 | P0 |
| P21-02 | IDeviceClient 抽象接口 — KCode 侧轻量调度层 | ✅ 已完成 | P0 |
| P21-03 | 设备配置管理 — 全局预置 + 任务绑定 + 临时选配三级规则 | 🛠️ 部分实现 | P0 |
| P21-04 | 多设备协议适配 — SSH/Telnet/ADB/Local 统一调用 | 🛠️ 部分实现 | P1 |
...
**待完成**:
- P21-01: device-executor 独立 CLI 未实现（当前嵌入 KCodePanel）
- P21-05: 设备日志输出在独立 Device tab，未融入对话时间线（`handleDeviceOutput` → `#device-output` div，不经过 `toolCallUpdate`）
- P21-06: Demo 子节点（依赖 Phase 20）
- P21-07: 自动自检未实现
...
| P21-02 | IDeviceClient 抽象接口 — KCode 侧轻量调度层 | ✅ 已完成 | P0 |
| P21-03 | 设备配置管理 — 全局预置 + 任务绑定 + 临时选配三级规则 | ✅ 已完成 | P0 |
| P21-04 | 多设备协议适配 — SSH/Telnet/ADB/Local 统一调用（DishCli + Local） | ✅ 已完成 | P1 |
...
## Phase 25: 开箱即用体验 — 安装 / Agent / 导入 / UI 统一

_目标：降低用户上手门槛，完善生态集成，统一 UI 视觉。_

| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P25-01 | 一键安装脚本 — 自动安装 kilo、opencode 等依赖环境 | ⬜ 未开始 | P0 |
| P25-02 | Claude Code + CodeBuddy CLI 支持 — Agent 注册与协议适配 | ⬜ 未开始 | P0 |
| P25-03 | 模型信息展示 — 在 Agent 名称旁显示当前模型 | ⬜ 未开始 | P0 |
| P25-04 | 导入任务入口 — 输入框左侧独立按钮组；删除「根据模板新建」| ⬜ 未开始 | P0 |
| P25-05 | 示例任务 — 预置示例，引导用户快速了解功能 | ⬜ 未开始 | P0 |
| P25-06 | 任务类型调整 — 五种：需求开发、代码评审、问题分析、缺陷分析、日志分析 | ⬜ 未开始 | P0 |
| P25-07 | 全局图标统一 — 我的任务 / 知识库 / 设置 / 小助手 页面统一使用 KCode 图标 | ⬜ 未开始 | P1 |
...
**调研步骤**:
1. 确定各 Agent 的 npm 包名和安装方式（kilo, opencode, claude-code, codebuddy）
2. 设计一键安装脚本路径和交互逻辑
...
### P25-02: Claude Code + CodeBuddy CLI

**涉及文件**: _待调研_

**调研步骤**:
1. 确认 Claude Code 与 CodeBuddy 是否支持 ACP 协议，或需要 HTTP/stdio 直连
2. 各自注册到 `AgentRegistry` 的配置模板
...
**调研步骤**:
1. 查 `AgentService` / `AcpClient` 中是否有模型信息可获取（`InitializeResponse` 包含 model）
2. WebView 侧 `#agent-dropdown-label` 处增加模型名显示
...
**调研步骤**:
1. 查 `chatPanelHtml.ts` 输入框布局，确认现有「根据模板新建」位置
2. 设计按钮分组：需求开发 / 代码评审 / ... 左侧导入按钮
...
**调研步骤**:
1. 设计 2-3 个示例任务（如"分析当前项目结构"、"实现一个简单功能"）
2. 确定示例数据存储方式（硬编码初始数据 / 首次启动自动生成）
...
**调研步骤**:
1. 查 `src/types/index.ts` 中 `TaskCategory` 类型定义
2. 查 `src/taskflow/templates.ts` 中分类注册，修改为五种类型
...
**调研步骤**:
1. 确认 `resources/kcode.png` 路径是否存在，查各 WebView 页面图标引用方式
2. 替换 myTasksApp、knowledge.ts、settingsApp、sidebar 中的图标
