...
| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P17-01 | 文件存储层 — TaskStore 迁移到 ~/.local/share/kcode | ✅ 已完成 | P0 |
...
**涉及文件**:
- `src/store/FileStorage.ts` — **新建**：文件存储类，`get`/`update`/`keys` 兼容 `vscode.Memento` 接口
- `src/store/TaskStore.ts` — 构造函数接受 `StorageBackend` 接口（统一 Memento/FileStorage）
- `src/extension.ts` — 初始化 FileStorage，迁移 workspaceState 旧数据
