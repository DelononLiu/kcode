...
| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P21-01 | device-executor 执行内核 — 独立 CLI 工具，完全解耦业务逻辑 | ⬜ 未开始 | P0 |
| P21-02 | IDeviceClient 抽象接口 — KCode 侧轻量调度层 | 📋 已调研 | P0 |
| P21-03 | 设备配置管理 — 全局预置 + 任务绑定 + 临时选配三级规则 | ⬜ 未开始 | P0 |
| P21-04 | 多设备协议适配 — SSH/Telnet/ADB/Local 统一调用 | ⬜ 未开始 | P1 |
| P21-05 | 流式执行输出 — 内嵌对话时序展示设备运行日志 | ⬜ 未开始 | P1 |
| P21-06 | Demo 作为标准执行子节点 — 依附主线流程，可重复/可跳过 | ⬜ 未开始 | P0 |
| P21-07 | 执行完成自动自检 + 结果回流任务流程 | ⬜ 未开始 | P1 |

### P21-02: IDeviceClient 抽象接口 + 连接管理 UI 重构

**涉及文件**:
- `src/types/index.ts` — 新增 `DeviceConfig`, `IDeviceClient` 接口, `DeviceConnection`, `DeviceType` 类型
- `src/device/DeviceClientFactory.ts` — **新建**：根据 `DeviceType` 创建对应 client 实例
- `src/device/LocalDeviceClient.ts` — **新建**：Local 协议实现（`child_process.exec`）
- `src/device/StubDeviceClient.ts` — **新建**：SSH/Telnet/ADB 占位实现（后续 P21-04 替换）
- `src/kcodeView/webview/device.ts` — 重写：mock → `_dvcVscode.postMessage` 与 Extension 通信；表单联动（type 切换隐藏/显示字段）；连接/断开/命令发送/输出渲染
- `src/kcodeView/templates/chatPanelHtml.ts` — 右侧浮层面板新增 `#tab-device` DOM（连接表单 + 终端视图 + 状态栏 + tab 按钮 `data-tab="device"`）
- `src/kcodeView/templates/chatPanelCss.ts` — 新增 Device tab 全部样式（表单/终端/状态指示器/输出行）
- `src/kcodeView/KCodePanel.ts` — 新增 `deviceClients: Map<string, IDeviceClient>` 字段；`router.on('deviceConnect'/'deviceDisconnect'/'deviceCommand')` 注册；`handleDeviceConnect()` 异步连接；`handleDeviceDisconnect()` 断开所有；`handleDeviceCommand()` 执行命令；`dispose()` 清理 device clients
- `src/kcodeView/webview/app.ts` — `initMessageHandler` 新增 `deviceConnected`/`deviceOutput`/`deviceStatus` 消息处理；`DOMContentLoaded` 调用 `initDeviceTab()`

**调研结果**:

**现有代码状态**:
- `src/kcodeView/webview/device.ts` — 完整 mock UI（SSH/Telnet 连接展示、终端输入框、断开/重连），但所有操作是前端 mock，无真实网络层
- 当前是**死代码** — `app.ts` 无任何 `device` 引用，未注册消息处理；`chatPanelHtml.ts` 右侧浮层面板 (`#right-panel`) 仅含 Diff/ACP Log 两 tab，无 `#tab-device` DOM
- `preview.ts:158-171` — `showDeviceFallback()` 渲染占位提示"设备连接功能将在后续版本实现"
- `KCodePanel.ts` — 无 `deviceConnect` 方法（PROJECT.md 文档过时）；`setupMessageHandler()` 无任何 device 相关消息注册
- 无 SSH/ADB/Telnet npm 依赖（`node-ssh`, `ssh2`, `adbkit` 均未安装）
...
| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P22-01 | 时序流内联排布 — 舍弃独立 Tab，所有操作按执行顺序排列 | ⬜ 未开始 | P0 |
| P22-02 | 视觉区分体系 — 前缀图标 + 高亮标题 + 等宽字体浅底色 + 彩色状态标识 | ⬜ 未开始 | P0 |
| P22-03 | 默认折叠策略 — 历史记录默认单行，失败/关键日志自动展开 | ⬜ 未开始 | P1 |
| P22-04 | 交互控制 — 点击展开/收起详情，新条目自动折叠非重点历史 | ⬜ 未开始 | P1 |
| P22-05 | 类型筛选按钮 — 顶部过滤只查看指定操作类型 | ⬜ 未开始 | P1 |
| P22-06 | 设备运行日志与工具调用视觉统一 — 融入同一条任务时间线 | ⬜ 未开始 | P1 |

---

## Phase 23: Windows 为核心的插件部署与运行策略

_目标：以 Windows VSCode 插件为主力载体，适配双场景（本地/Remote），本机统一管控任务流程与设备调度。_

| 任务 | 说明 | 状态 | 优先级 |
|------|------|------|--------|
| P23-01 | Windows VSCode 插件打包发布 — vsix 构建与部署 | ⬜ 未开始 | P0 |
| P23-02 | 本机 + Remote 双场景兼容 — 远程工作区代码编辑，本机统一管控 | ⬜ 未开始 | P0 |
| P23-03 | 运行权责划分 — 远程只做文件操作，本机负责流程/设备/日志全局管控 | ⬜ 未开始 | P0 |
| P23-04 | Windows 终端适配 — 盘符路径/PowerShell/CMD/WSL 多终端 | ⬜ 未开始 | P1 |
| P23-05 | 内网组网优化 — 免密连接/外设调用/端口进程管理 | ⬜ 未开始 | P1 |
