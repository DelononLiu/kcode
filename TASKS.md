...
### P8-03: 阶段分发提示词 — buildTaskPrompt() 按 phase 分派

**涉及文件**:
- `src/kcodeView/KCodePanel.ts` — 重写 `buildTaskPrompt()` 按 `task.phase` 分派 4 套提示词

**调研结果**:
- `KCodePanel.ts:402-409` — 当前 `buildTaskPrompt()` 单一提示词模板，不区分阶段
- design doc §五定义完整的行为约束 + `<TASK_UPDATE>` 格式提示
- 每轮只注入当前阶段上下文 + confirmedItems，节约 token

**实现说明**:
- `buildTaskPrompt()` 按 `task.phase` 分派：

  | phase | 注入内容 |
  |-------|---------|
  | `goal` | goal 阶段行为约束（只归纳确认，不写代码）+ `<TASK_UPDATE>` 格式说明 + 用户输入 |
  | `plan` | plan 阶段约束 + 已锁定 goal + `<TASK_UPDATE>` 格式 + 用户输入 |
  | `execute` | execute 阶段约束 + 已锁定 goal + 已锁定 planSteps + `<TASK_UPDATE>` 格式 + 用户输入 |
