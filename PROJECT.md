...
> **当前阶段：🟩 Level 4 — 能吃** — 团队完全切换到 KCode 开发 KCode，不再用其他 AI 工具。

---

### Phase 12: 卡片UI堆叠

**目标**：减少工具卡片视觉干扰，同一时间只展示当前活跃卡片的内容，历史相邻卡片自动收拢为仅显示头部标题。

**当前问题**：AI 在一次回复中可能产生多个连续的工具调用（think → read → bash → write → ...），每个渲染为一个独立卡片。所有卡片同时展开占据大量屏幕空间，用户实际只关心当前正在执行或刚完成的工具卡片。

**堆叠规则**：

- **适用卡片**：所有工具卡片 — `thinking`、`bash`/`command`/`terminal`、`read`/`write`/`edit`、`grep`/`glob`/`search`
- **仅堆叠相邻卡片**：只有在同一 agent 消息中**连续追加**的工具卡片才触发堆叠。如果中间插入了普通文本输出，则前后不堆叠
- **活跃卡片**：只有**最后一张**（最新/当前活跃的）卡片显示完整 body 内容
- **历史卡片**：之前的相邻卡片全部强制折叠，仅保留 `msg-card-header`（图标 + 工具名称标题）
- **用户可展开**：点击任意历史卡片头部可临时展开查看详情，展开后该卡片成为新的"活跃"卡片，其他卡片重新折叠
- **新卡追加时自动堆叠**：当新的工具卡片追加到同一 agent 消息时，自动折叠之前的所有相邻卡片

**验收标准**：AI 回复中出现一组连续工具卡片时，只有最后一张展开，其余仅显示头部标题；中间有文本输出时，前后的卡片各自独立堆叠；用户可点击展开历史卡片查看详情。

---

### Phase 13: Todo 卡片

**目标**：在对话中支持结构化待办清单卡片 — AI 输出 todo 列表，用户可勾选确认完成状态。

**设计**：

- **消息类型**：新增 `'todo'` ChatMessage 子类型
- **数据模型**：
  ```typescript
  interface TodoItem {
      id: string;
  ...
  ```
- **渲染**：卡片形式展示 checkbox 列表 + 底部进度指示（3/5 已完成）+ 进度条
- **交互**：用户点击 checkbox 勾选/取消，状态通过 postMessage 同步到 extension，持久化到 store
- **AI 协议**：通过 `<TODO_UPDATE>` 协议输出 todo 项的新增或状态更新
- **应用场景**：AI 在执行阶段输出的步骤清单、验收阶段的自查清单、用户要求展示的待办项

**验收标准**：AI 可输出结构化 todo 卡片，含可勾选清单项和进度指示；用户勾选状态持久化保存。
