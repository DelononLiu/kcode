...
已通过 `acp-mvp` 项目验证完整协议流程：

| 步骤 | 状态 |
|------|------|
| `opencode acp --port 0 --cwd` stdio 子进程启动 | ✅ |
| `initialize` 协议版本协商 (v1) | ✅ |
| `session/new` 会话创建 | ✅ |
| `session/prompt` 流式文本响应 (agent_message_chunk) | ✅ |
| `session/update` 工具调用通知 (tool_call) | ✅ |
| `session/update` 执行计划 (plan) | ✅ |
| `session/update` Agent 思考 (agent_thought_chunk) | ✅ |
| `session/cancel` 取消请求 | ✅ |
| 断开连接 (SIGTERM) | ✅ |

OpenCode v1.14.33 报告的 capabilities：
- `loadSession: true`
...
│   ├── client-handler.ts      # Client 侧处理器
│   ├── fake-agent-server.ts   # Fake Agent（测试用）
│   ├── test-opencode.ts       # 对真实 OpenCode 的端到端测试
│   ├── test-fake.ts           # 对 Fake Agent 的测试
│   ├── repl.ts                # REPL 交互模式
│   └── types.ts               # 类型定义
├── package.json
└── tsconfig.json
...
cd acp-mvp
npm install
npx tsx src/test-fake.ts "你好"   # 无需 OpenCode
npx tsx src/test-opencode.ts "Hello" /tmp  # 需要 OpenCode
npx tsx src/repl.ts               # REPL 交互模式
npx tsx src/repl.ts --fake        # REPL + Fake Agent
```
