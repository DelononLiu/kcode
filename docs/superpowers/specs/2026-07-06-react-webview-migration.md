...
2. **核心优先** — 先迁移对话区（消息列表 + 输入框 + 阶段卡片 + 折叠），同时覆盖**任务模式**和**小助手模式**（两者共享消息渲染和流式处理，仅阶段卡片为任务模式专有）。侧边栏/知识/设置等后续
...
### Step 6: 测试
- 任务模式全流程（goal → plan → execute → self_verify → review，含阶段卡片）
- 小助手模式全流程（纯对话 + 流式 + 折叠，无阶段卡片）
- 两种模式切换（任务 ↔ 助理）
- 折叠功能（两种模式都验证）
- 暗色/亮色切换
- 输入框/发送/停止
