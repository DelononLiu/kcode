...
| **左侧任务列表** | ✅ 文件树+任务 | ✅ 仅任务(按工作区) | ⚠️ 已实现(无工作区分组) | 需实现 Open Workspace + 工作区分组 |
...
| **文件管理** | ✅ 内置文件树 | ✅ 复用VS Code Explorer | ⚠️ Agent写文件未通知Editor实时更新 | 需实现文件变更后自动打开/刷新 |
...
| 文件同步 | Agent writeTextFile 后未通知 VS Code Editor 实时刷新 | ⬜ 待实现 |

### ❌ 未实现 (MVP 范围内待实现)

| 模块 | 缺失详情 | 优先级 |
|------|----------|--------|
| **工作区管理** | 无 `openWorkspace.ts`，TaskStore 无 workspace 字段，sidebar 无 Open Workspace 按钮，无工作区分组展示 | **高** |
| **设置页面** | 无自有设置 UI (API Key/Agent Path)；openSettings 仅跳转 VS Code 原生设置 | **高** |
| **状态栏** | 底部无用户信息/设置入口状态栏 (仅有 Agent 状态标签) | 中 |
