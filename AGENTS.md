...
> **📖 先读 `.kilo/project-index.md`** — 包含完整的文件导出、消息协议、数据流索引，避免重复全局搜索。
...
| 顺序 | 文件 | 目的 |
|------|------|------|
| 1 | `.kilo/agents.md` | 本指南，了解工作流规范 |
| 2 | `.kilo/project-index.md` | 项目模块索引、文件导出、消息协议 |
| 3 | `.kilo/tasks.md` | 定位具体任务条目，获取「涉及文件」「调研步骤」「关键函数」 |

**禁止**：不要 glob 搜索整个工程，不要 grep 全局搜关键字。所有文件定位应来自 `project-index.md` 或 task 的「涉及文件」列表。
...
**调研阶段**（涉及文件为空时）：
- 只读 `project-index.md` + 通过索引定位到的目标文件
...
- 如果新增/移动/删除文件 → 更新 `AGENTS.md` 项目结构图 + `project-index.md` 目录结构
- 如果新增消息类型 → 更新 `project-index.md` 通信协议部分
...
⬜ 未开始
    │  涉及文件为空
    ▼
🔍 调研中  ← AI 读 project-index.md 定位目标，填充「涉及文件」
    │
    ▼
...
**涉及文件**: _待调研_     ← 新任务此处为空
**调研步骤**:
1. 读 project-index.md → 定位 KCodeSidebarProvider、sidebar.ts
2. 打开两个文件确认渲染逻辑
