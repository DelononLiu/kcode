...
```
1. 读索引  ── 先读 .kilo/project-index.md 定位目标文件（替代全局搜索）
2. 定计划  ── 用 EnterPlanMode 设计实现方案，待用户确认
3. 实现    ── 按计划编写代码，编译通过 (npm run compile)
4. 验收    ── 验证构建成功，确认改动符合预期
5. 更新索引 ── 如果新增/移动/删除文件、新增消息类型或接口，更新 project-index.md
```

### 各步骤说明

**步骤 1 — 读索引**
- 先读 `.kilo/project-index.md`（代码库索引）和 `AGENTS.md`（工作流指南）
- 根据任务描述，从索引中定位到具体文件路径和方法名
- 直接读取目标文件，不再做全局 glob/grep 搜索
- 如果索引不完整或过时，先在步骤 5 更新它

**步骤 2 — 定计划**
- 使用 `EnterPlanMode` 工具进入计划模式
- 从索引定位的目标文件开始，深入阅读相关代码
- 设计实现方案，写 plan 文件
